...
import { Exhibit, Gallery } from '../models/types.js';
...
interface ChatResponse {
  ...
  source: 'athena-art-ai' | 'gemini-flash' | 'groq-llama' | 'local-art-expert';
}
...
export async function askAIGuide(req: ChatRequest): Promise<ChatResponse> {
  ...
  let targetExhibit: Exhibit | undefined = exhibitId
  ...
  if (!targetExhibit) {
    targetExhibit = dbStore.exhibits.find(e => {
  ...
  const relatedExhibits = dbStore.exhibits
    .filter(e => e.exhibitId !== targetExhibit?.exhibitId && (
      (targetExhibit && e.galleryId === targetExhibit.galleryId) ||
 ...
 */
function generateAutonomousArtAI(
  query: string,
...
Digital passes can be booked directly in our Tickets tab!`;
  ...
  if (q.includes('gallery') || q.includes('floor') || q.includes('map') || q.includes('where is')) {
    const matchedGallery = dbStore.galleries.find(g =>
    ...
    if (matchedGallery) {
      const artCount = dbStore.exhibits.filter(e => e.galleryId === matchedGallery.galleryId).length;
...
Which gallery would you like to navigate to?`;
  ...
  if (q.includes('recommend') || q.includes('must see') || q.includes('highlight') || q.includes('best') || q.includes('first') || q.includes('top')) {
    const highlights = dbStore.exhibits.filter(e => e.highlight || e.featured).slice(0, 4);
...
• Location: ${exhibit.location} (${exhibit.galleryId})`;
    ...
    if (q.includes('near') || q.includes('next') || q.includes('companion') || q.includes('around') || q.includes('same room')) {
      const neighbors = dbStore.exhibits
...
function generateTamilAIResponse(q: string, exhibit?: Exhibit): string {
  if (exhibit) {
...
function generateHindiAIResponse(q: string, exhibit?: Exhibit): string {
  if (exhibit) {
...
function generateFrenchAIResponse(q: string, exhibit?: Exhibit): string {
  if (exhibit) {
...
function generateSpanishAIResponse(q: string, exhibit?: Exhibit): string {
  if (exhibit) {
