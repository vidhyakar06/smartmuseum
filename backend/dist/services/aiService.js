...
async function askAIGuide(req) {
    ...
    const currentExhibit = exhibitId
    ...
    const relatedExhibits = db_js_1.dbStore.exhibits
        .filter(e => e.exhibitId !== currentExhibit?.exhibitId && (e.galleryId === currentExhibit?.galleryId ||
...
function generateLocalAIResponse(query, exhibit, lang = 'en') {
    ...
    if (q.includes('near') || q.includes('next') || q.includes('nearby') || q.includes('around') || q.includes('recommend') || q.includes('similar')) {
        const nearby = db_js_1.dbStore.exhibits
